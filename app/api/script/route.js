...
const REMOTE_BROWSER_WS_ENDPOINT =
	process.env.PLAYWRIGHT_WS_ENDPOINT ?? process.env.BROWSERLESS_WS_ENDPOINT;
...
export async function GET() {
	...
	try {
		...
		stage = "find login fields";
		const usernameField = page.getByLabel(/username/i).first();
		const passwordField = page.getByLabel(/password/i).first();

		await usernameField.waitFor({ state: "visible", timeout: DEFAULT_TIMEOUT });
		await passwordField.waitFor({ state: "visible", timeout: DEFAULT_TIMEOUT });

		stage = "submit login";
		await usernameField.fill(LOGIN_VALUE, { timeout: DEFAULT_TIMEOUT });
		await passwordField.fill(PASSWORD_VALUE, { timeout: DEFAULT_TIMEOUT });
		await Promise.all([
			page.getByRole("button", { name: /login/i }).click({ timeout: DEFAULT_TIMEOUT }),
			page.waitForLoadState("domcontentloaded", { timeout: DEFAULT_TIMEOUT }).catch(() => null)
		]);

		stage = "open gradebook";
		await page.goto(GRADEBOOK_URL, {
			waitUntil: "domcontentloaded",
			timeout: DEFAULT_TIMEOUT
		});

		stage = "wait for gradebook table";
